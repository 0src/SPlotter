		...
		for (unsigned long long n = 0; n < local_nonces;)
		{
			if (n + 4 <= local_nonces)
			{
				...
				for (size_t i = PLOT_SIZE; i > 0; i -= HASH_SIZE)
				{
					...
					len = PLOT_SIZE + 16 - i;
					if (len > HASH_CAP)   len = HASH_CAP;
					avx2_mshabal(mx, &gendata1[i], &gendata2[i], &gendata3[i], &gendata4[i], len);
					avx2_mshabal_close(mx, 0, 0, 0, 0, 0, &gendata1[i - HASH_SIZE], &gendata2[i - HASH_SIZE], &gendata3[i - HASH_SIZE], &gendata4[i - HASH_SIZE]);
				}

				avx2_mshabal_init(mx, 256);
				avx2_mshabal(mx, gendata1, gendata2, gendata3, gendata4, 16 + PLOT_SIZE);
				avx2_mshabal_close(mx, 0, 0, 0, 0, 0, final1, final2, final3, final4);
